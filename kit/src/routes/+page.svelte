<script lang="ts">
  ...
  const load = () => {
    ...
    categories.forEach(
      (tempcat) => (tempcat.weight = Math.floor(per * 100) / 100)
    );
  ...
  function Line(text, name, score, outOf, category) {
    if (text == "") {
      ...
      this.outOf = outOf;
      this.percent =
        (((parseFloat(this.score) / parseFloat(this.outOf)) * 10000) >> 0) /
        100;
      ...
      this.outOf = this.gradetext.substr(
        this.gradetext.indexOf("/") + 1,
        this.gradetext.length
      );
      this.percent =
        (((parseFloat(this.score) / parseFloat(this.outOf)) * 10000) >> 0) /
        100;
  ...
  function updateGrade() {
    ...
    categories.forEach((cat) => {
      cat.weight =
        parseFloat(document.getElementsByClassName("inh")[temp].value) / 100;
      temp++;
    });
    calcGrade();
  ...
  function updateAssignments() {
    categories.forEach((cat) => {
      let temp = 0;
      cat.assignments.forEach((ass) => {
        ass.score = document.getElementsByClassName(cat.name + "in")[
          temp
        ].value;
        ass.outOf = document.getElementsByClassName(cat.name + "out")[
          temp
        ].value;
        ass.percent =
          (((parseFloat(ass.score) / parseFloat(ass.outOf)) * 10000) >> 0) /
          100;
        document.getElementsByClassName(cat.name + "percent")[temp].innerHTML =
          "" + ass.percent + "%";
        temp++;
      });
    });
  ...
  function addAssignment(cat) {
    ...
    clonedRow.cells[0].innerHTML =
      "<input type='text' value='New Assignment " + assnum + "'>";
    clonedRow.cells[1].childNodes[0].addEventListener(
      "change",
      updateAssignments
    );
    clonedRow.cells[2].childNodes[0].addEventListener(
      "change",
      updateAssignments
    );
  ...
  function updateCategoryGrades() {
    for (let i = 0; i < categories.length; i++) {
      ...
      document.getElementsByClassName(
        categories[i].name + "grade"
      )[0].innerHTML =
        "<strong>" + Math.round((points / total) * 10000) / 100 + "%</strong>";
    }
  }
</script>
...
<main>
  <center>
      ...
      <table style="width: 90%">
        <tr style="margin-bottom:0.5px">
          ...
          <td style="text-align:right; font-weight:bold; width:25%"
            ><button on:click={load}>load</button></td
          >
          <td style="width:25%" />
          <td />
        </tr>
    ...
    <div id="t">
      <h2 style="visibility:hidden;" id="finalgrade">
